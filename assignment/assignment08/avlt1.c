...
static NODE *_insert( NODE *root, NODE *newPtr, int (*compare)(const void *, const void *)){
	...
	if(compare(root->dataPtr, newPtr ->dataPtr) > 0){
...
static int getHeight( NODE *root){
    ...
    root->height = max(left_height, right_height) + 1;	//이러면 root아래의 모든 노드도 갱신될듯;
    return root -> height;
	}
}
static NODE *_smallest_node( NODE *root){
...
static NODE *rotateLeft( NODE *root){
	...
	getHeight(root);	//Height 갱신	
	return root;
}

//root의 balancefactor 계산		
static int balancefactor( NODE *root){
	if(root == NULL)
		return 0;
